...
//
void ADXL362::begin(int16_t slaveSelectPin) {
	...
	delay(10);
	SPIwriteOneRegister(0x2A, 0x00);   
	SPIwriteOneRegister(0x2B, 0x00);   
  // turn on Autosleep bit
    byte POWER_CTL_reg = SPIreadOneRegister(0x2D);
    POWER_CTL_reg = POWER_CTL_reg | (0x04);				// turn on POWER_CTL[2] - Autosleep bit
    SPIwriteOneRegister(0x2D, POWER_CTL_reg);
...
void ADXL362::setupACActivityInterrupt(int16_t threshold, byte time){
    ...
    SPIwriteOneRegister(0x27, 0x3F); // Referenced Activity, Referenced Inactivity, Loop Mode  
	...
	// turn on activity interrupt
	byte INT_Reg = SPIreadOneRegister(0x2A);  // Read current reg value
	byte ACT_INACT_CTL_Reg = SPIreadOneRegister(0x27);  // Read current reg value
...
#ifdef ADXL362_DEBUG
	Serial.print("INT set to ");  	Serial.print(SPIreadOneRegister(0x2A));
